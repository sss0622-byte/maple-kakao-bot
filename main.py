...
def basic_card(title: str, description: str, buttons: list) -> dict:
    return {
        "version": "2.0",
        "template": {
            "outputs": [{
...
def list_card(header: str, items: list, buttons: list = None) -> dict:
...
def scouter_url(path: str, char_name: str) -> str:
    """한글 닉네임 URL 인코딩 처리"""
    return f"{SCOUTER_BASE}/{path}?name={quote(char_name)}"
...
async def character_info(req: Request):
    ...
    buttons = [
        link_button("환산주스탯 보기",  scouter_url("result", name)),
        link_button("보스컷 분석",      scouter_url("optimizer", name)),
        link_button("스펙업 순서",      scouter_url("spec-order", name)),
    ]
...
async def boss_info(req: Request):
    ...
    items = [
        {"title": "📊 효율·보스컷 분석",  "description": "환산 기준 보스 입장 가능 여부",
         "action": "webLink", "webLinkUrl": scouter_url("result", char_name)},
        {"title": "🎯 보스세팅 최적화",    "description": "주간 보스 최적 선택 추천",
         "action": "webLink", "webLinkUrl": scouter_url("optimizer", char_name)},
        {"title": "💰 주간 보스 정산",      "description": "결정석 수익 계산",
         "action": "webLink", "webLinkUrl": scouter_url("boss-income", char_name)},
...
async def spec_info(req: Request):
    ...
    items = [
        {"title": "📈 스펙업 순서",        "description": "투자 우선순위 추천",
         "action": "webLink", "webLinkUrl": scouter_url("spec-order", char_name)},
        {"title": "🔷 헥사코어 순서",       "description": "6차 코어 강화 순서 분석",
         "action": "webLink", "webLinkUrl": scouter_url("hexa", char_name)},
        {"title": "⭐ 스타포스 효율",        "description": "강화 단계별 효율 분석",
         "action": "webLink", "webLinkUrl": scouter_url("starforce", char_name)},
        {"title": "💎 잠재·추가옵션 효율",   "description": "큐브 세팅 효율 분석",
         "action": "webLink", "webLinkUrl": scouter_url("cube-fire", char_name)},
        {"title": "🗺️ 사냥컷 분석",         "description": "사냥터별 원킬컷 확인",
         "action": "webLink", "webLinkUrl": scouter_url("huntresult", char_name)},
    ]
...
async def dojang_info(req: Request):
    ...
    buttons = [link_button("무릉 랭킹 보기", scouter_url("result", char_name))]
...
async def ranking_info(req: Request):
    ...
    items = [
        ...
        {"title": "🔍 내 캐릭터 환산",      "description": f"{char_name or '캐릭터명 입력'}의 환산 확인",
         "action": "webLink",
         "webLinkUrl": scouter_url("result", char_name) if char_name else SCOUTER_BASE},
